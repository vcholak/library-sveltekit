	...
	<div class="container mx-auto">
		<div class="py-8">
			<div class="flex flex-wrap flex-row mb-1 sm:mb-0 justify-between w-full">
				<h2 class="text-2xl leading-tight md:pr-0 text-white">Book Copies</h2>
				<div>
					<a href="/admin/copies/create" class="px-4 py-2 text-base font-semibold rounded-lg bg-gray-600 text-white hover:bg-purple-700">New Book Copy</a>
				</div>
				<div class="text-end">
			...
			<div class="py-4">
				<div class="max-w-full overflow-x-auto shadow rounded-lg">
					...
					{#if bookCopies}
						{#each bookCopies as bookCopy}
							<tr>
								<td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
									<p class="text-gray-900 whitespace-no-wrap">
										<a href={`/admin/copies/${bookCopy.id}`} class="hover:text-blue-500">
											<span>{bookCopy.book?.title}</span>
										</a>
									</p>
								</td>
							</tr>
					...
					<Pagination items={data.bookCopies} bind:trimmedItems={bookCopies} />
				</div>
			</div>
		</div>
	</div>
